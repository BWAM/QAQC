...
EB$result_value <- ifelse(grepl("U", EB$lab_qualifiers),
                          0,
                          EB$result_value)
...
#create pass column
EB$pass <- ifelse(EB$result_value > EB$quantitation_limit,
                  "fail",
                  "pass")
#format date
...
data$EBpass <-
  ifelse(is.na(data$blank),
         "A",
         ifelse(
           data$result_value <= 3 * (data$blank),
           "R",
           ifelse(
             data$result_value <= 5 * (data$blank),
             "T",
             ifelse(
               data$result_value <= 10 * (data$blank),
                    "J",
                    "A"
               )
           )
