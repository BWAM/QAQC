...
if (nrow(dup) > 0) {
  data$validator_qualifiers <-
    ifelse((
      data$labpass == "R" |
        data$spikepass == "R" |
        data$duppass == "R" |
        data$EBpass == "R" | data$pairpass == "R" | data$HTpass == "R"
    ),
    "R",
    ifelse(data$EBpass == "T", "T",
           ifelse((data$labpass == "J" | data$EBpass == "J"),
                  "J",
...
## For lab errors
#R flags
data$interpreted_qualifiers <-
  ifelse(
    data$interpreted_qualifiers == "Normal",
    ifelse(data$labpass == "R", "lab error", "Normal"),
    ifelse(
      data$labpass == "R",
      paste(data$interpreted_qualifiers, " ; lab error", sep = ""),
      data$interpreted_qualifiers
    )
  )
#J flags
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      ...
      paste(
        data$interpreted_qualifiers,
        " ; estimated value due to lab error",
        sep = ""
      ),
      data$interpreted_qualifiers
    )
...
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      data$EBpass == "R",
      paste(data$interpreted_qualifiers, " ; Equipment Blank error", sep = ""),
      data$interpreted_qualifiers
    )
  )
#J flags
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      data$EBpass == "J",
      "estimated value due to Equipment Blank error",
      "Normal"
    ),
    ifelse(
      data$EBpass == "J",
...
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      data$spikepass == "R",
      paste(data$interpreted_qualifiers, " ; Accuracy error", sep = ""),
      data$interpreted_qualifiers
    )
  )
...
if (nrow(dup) > 0) {
  data$interpreted_qualifiers <-
    ifelse(
      ...
      ifelse(
        data$duppass == "R",
        paste(data$interpreted_qualifiers, " ; Precision error", sep = ""),
        data$interpreted_qualifiers
      )
    )
}
...
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      data$pairpass == "R",
      "Result should be less than the total",
      "Normal"
    ),
    ifelse(
      data$pairpass == "R",
...
data$interpreted_qualifiers <-
  ifelse(
    ...
    ifelse(
      data$HTpass == "R",
      paste(data$interpreted_qualifiers, " ; Holding Time error", sep = ""),
      data$interpreted_qualifiers
    )
  )
...
data$interpreted_qualifiers <-
  ifelse(data$interpreted_qualifiers == "Normal",
         'Accepted',
         data$interpreted_qualifiers)
